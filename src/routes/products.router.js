...
router.post('/', uploader.single('file'), async (req, res) => {
    try {
        ...
        if (!req.body) {
            return res.status(400).json({ status: 'error', message: 'Product no can be created without properties' })
        }
...
router.put('/:pid', async (req, res) => {
    try {
        ...
        res.status(200).json({ status: 'success', message: `Updating the product: ${productFind.title}` })
...
router.delete('/:pid', async (req, res) => {
    try {
        ...
        req.app.get('socketio').emit('updatedProducts', updatedProducts)
        res.status(200).json({ status: 'success', message: `Product with id ${pid} removed successfully`, products:  updatedProducts })
